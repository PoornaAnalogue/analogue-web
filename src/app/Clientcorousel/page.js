...
export default function ClientCarousel() {
  ...
  const companies = [
    ...
    { logo: "/healrlogo.png", phone: "/healrimg.png" },
  ];
 ...
 const steps = [
  ...
  return (
    <div className="w-full">
        ...
        >
          {companies.concat(companies).map((c, i) => (
            <SwiperSlide key={`logo-${i}`} onClick={() => onLogoClick(i % companies.length)}>
              <div className={`flex justify-center transition-transform duration-300 ${activeIndex === i % companies.length ? "scale-110 opacity-100" : "opacity-50"}`}>
                <Image src={c.logo} alt={`Logo ${i}`} width={120} height={50} className="object-contain" />
              </div>
            </SwiperSlide>
          ))}
        </Swiper>
          ...
          >
            {companies.concat(companies).map((c, i) => (
              <SwiperSlide key={`phone-${i}`}>
                <div className={`phone-wrapper flex justify-center ${c.phone.includes("/healrimg.png") ? "healr-slide" : ""}`}>
                  <Image src={c.phone} alt={`Phone ${i}`} width={260} height={520} className="object-contain" />
                </div>
              </SwiperSlide>
            ))}
          </Swiper>
      ...
      {/* ==================== Puzzle Steps ==================== */}
      <div ref={containerRef} className="h-screen w-full flex items-center justify-center bg-[#071637] text-white overflow-hidden">
