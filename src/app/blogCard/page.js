  ...
  return (
    <div className="  max-w-xl mx-auto bg-white rounded-xl shadow-md overflow-hidden  hover:shadow-lg transition-shadow duration-300  ">
      ...
      >
        ...
        height={150}
          alt={title}
          className={` w-full h-70  transition-transform duration-300 object-contain ${isHovered ? 'scale-110' : 'scale-100'}`} />
      </div>

      <div className="p-3 px-6 flex w-full">
        <div className=''>
          <h3 className="text-gray-800 text-lg xss:text-sm  sm:text-base md:text-lg 2xl:text-[1.1rem] leading-5 font-medium mb-2">{title}</h3>
        <p className="text-gray-600 text-xs xss:text-xs sm:text-sm lg:text-subbody 3xl:text-base mb-4 leading-5 ">{description}</p>
