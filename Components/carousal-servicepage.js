...
export default function CarousalServicePage() {
  ...
  const letterVariants = {
    hidden: { color: "#a5a7adff" },
    visible: { color: "#2b56b3ff" },
  };
  ...
  return (
    <div className="w-full px-4 sm:px-8 md:px-12 overflow-x-hidden lg:px-16">
      ...
      <h2
        id="featured-heading"
        className="font-['Urbanist'] text-2xl xss:text-[1.1rem] xs:text-[1.2rem] sm:text-[1.3rem] md:text-[1.5rem]  xl:text-[1.8rem] 3xl:text-[2.2rem] font-semibold text-black leading-snug text-center text-[#7B7E86] my-5 sm:mb-8 md:mb-10 xl:py-5"
      >
        {heading.map((char, i) => (
          <motion.span
            key={i}
            ...
            <SwiperSlide key={`logo-${i}`} onClick={() => onLogoClick(i)}>
              ...
              >
                <Image
                  ...
                  width={120}
                  height={100}
                  className={`object-contain w-[50px] h-[30px] sm:w-[70px] sm:h-[40px] md:w-[90px] md:h-[55px] lg:w-[110px] lg:h-[70px] transition-all duration-300 ${activeIndex === i % companies.length ? "grayscale-0" : "grayscale"}`} />
              </div>
            </SwiperSlide>
          ))}
        </Swiper>
