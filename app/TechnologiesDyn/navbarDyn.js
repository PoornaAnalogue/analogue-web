...
export default function NavbarDyn() {
  ...
  items: technologies.map((tech) => ({
    // words: tech.highlightWords,
    name : tech.highlightWords.join(" "),
    link: `/${tech.slug}`, // dynamic path
  })),
},
{
  label: "Industries",
  items: industriesData.map((indus) => ({
    name: indus.name,
    link: `/${indus.slug}`, // dynamic path
  })),
},
    // {
    //   label: "Industries",
  ...
  return (
    <div className="Navbar-main overflow-visible !bg-[#FAFBFF] h-screen border-2 border-red-400">
      {/* Top Strip */}
