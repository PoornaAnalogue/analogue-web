...
useEffect(() => {
  ...
  const handleScroll = () => {
    ...
    if (inView && !alreadySnapped && !isPuzzleCompleted) {
      // Prevent flickering by setting states immediately before snap
      setIsSnapping(true);

      // Snap immediately with auto behavior
      ...
      requestAnimationFrame(() => {
        ...
        // Set puzzle direction and initial step based on scroll direction
        if (direction === "down") {
          setPuzzleDirection('forward');
          // Only reset to minStep if we're not already in a puzzle state
          if (!hasEnteredSection.current) {
            setCurrentStep(minStep); // Start at step 1 for forward
          }
        } else {
          setPuzzleDirection('reverse');
          // Only reset to maxStep if we're not already in a puzzle state
          if (!hasEnteredSection.current) {
            setCurrentStep(maxStep); // Start at step 9 for reverse
          }
        }
      ...
      setPuzzleDirection('forward');
      // Reset to initial state when leaving section completely
  ...
  return (
    <div id="puzzle-container" ref={headingRef} className="  lg:min-h-0  flex  flex-col justify-start items-center snap-center">
      {!isSnapping && (
    <div className="sticky-wrapper  ">
      <h2
        id="featured-heading"
        className=" font-['Urbanist'] text-2xl xss:text-3xl sm:text-3xl mt-5 md:text-5xl lg:text-6xl text-center text-[#7B7E86] mb-6 sm:mb-5 md:mb-10 xss:my-5 xs:my-3 xl:mb-3 2xl:py-3 "
      >
        {heading.map((char, i) => (
          <motion.span
            key={i}
      ...
      <div
        className="  w-full min-h-0 sm:min-h-screen lg:min-h-0   grid grid-cols-1 lg:grid-cols-2 items-start bg-[#071637] text-white py-10 lg:py-20 sm:py-10 px-4 lg:px-12 gap-8  xss:py-5 xs:py-0 xl:py-5 2xl:py-15"
      >
        {/* Left Content */}
          ...
          <AnimatePresence mode="wait">
            {currentStep > 0 && (
              <motion.div
                ...
                className="mt-6 xl:mt-2  xss:mt-2 sm:mt-6 space-y-4 pr-2"
              >
                <h4 className="font-weight-400 font-semibold text-[#0D6EFD] text-lg xss:text-sm sm:text-base md:text-lg 2xl:text-[1.5rem] xl:mb-2  xss:mb-1 mb-2">
                  Step {steps[(selectedStep ?? currentStep) - 1].id}: {" "}
                  {steps[(selectedStep ?? currentStep) - 1].title}
                </h4>
                <p className="text-neutral-300 text-xs xss:text-xs sm:text-sm lg:text-subbody 3xl:text-base xss:leading-5 sm:leading-6 text-[#7B7E86] xss:pt-3 xs:pt-0 ">
